...
export const PremiumPackage: React.FC<PremiumPackageProps> = ({ userEmail, session }) => {
    ...
    return (
      <div className="flex flex-col items-center gap-4 py-12 text-center">
        ...
        <button
          ...
          className="px-6 py-2 rounded-full bg-[#7c6aff]/10 border border-[#7c6aff]/30 text-[#a78bfa] text-sub font-bold hover:bg-[#7c6aff]/20 transition-all"
        >
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── PLANS ── */}
      <div>
        <p className="text-sub font-semibold text-text-muted mb-3">{t('premium_pkg_choose_plan')}</p>
          ...
          {PLANS.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => setSelectedPlan(p.id)}
                ...
                {/* Badge */}
                {badgeLabel && (
                  <span className={`absolute -top-2.5 left-1/2 -translate-x-1/2 text-tiny font-bold px-2 py-0.5 rounded-full text-white whitespace-nowrap ${p.badge === 'best_value' ? 'bg-[#7c6aff]' : 'bg-emerald-500'}`}>
                    {badgeLabel}
                  </span>
                )}
                ...
                {/* Discount */}
                {p.discount && (
                  <span className="absolute top-2 right-2 text-tiny font-bold text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded-full">
                    -{p.discount}%
                  </span>
                )}

                {/* Days */}
                <p className={`text-title font-black leading-none mb-0.5 ${isSelected ? 'text-[#a78bfa]' : 'text-text-primary'}`}>
                  {p.days} {t('premium_pkg_days')}
                </p>

                {/* Price */}
                <p className="text-caption font-semibold text-text-muted mt-1">
                  {p.price.toLocaleString()} {t('premium_pkg_currency')}
                </p>

                {/* Per day */}
                <p className="text-tiny text-text-muted/70 mt-0.5">
                  ≈ {Math.round(p.price / p.days).toLocaleString()} {t('premium_pkg_per_day')}
                </p>

                {isSelected && (
                  <div className="absolute bottom-2 right-2 w-4 h-4 rounded-full bg-[#7c6aff] flex items-center justify-center text-white text-tiny font-black">✓</div>
                )}
              </button>
            );
          })}
      ...
      {/* ── PAYMENT METHOD ── */}
      <div>
        <p className="text-tiny font-black text-text-muted uppercase tracking-widest mb-2">{t('premium_pkg_payment_method')}</p>
        <div className="grid grid-cols-2 gap-2">
          {PAYMENT_METHODS.map(pm => {
            ...
            return (
              <div key={pm.id} onClick={() => setSelectedPayment(pm.id)}
                ...
                {isSelected && <div className="absolute top-2 right-2 w-4 h-4 rounded-full bg-[#7c6aff] flex items-center justify-center text-white text-tiny font-black">✓</div>}
                <img
                  src={pm.logo}
                  alt={pm.name}
                  width={40}
                ...
                />
                <p className="text-sub font-black text-text-primary">{pm.name}</p>
                <div className="flex items-center gap-1.5">
                  <span className="text-tiny text-text-muted font-mono">{pm.number}</span>
                  <button onClick={e => { e.stopPropagation(); handleCopy(pm.number, pm.id); }}
                    className={`text-tiny font-bold px-1.5 py-0.5 rounded-full border transition-colors ${copied === pm.id ? 'border-emerald-500/40 text-emerald-400' : 'border-[var(--border-color)] text-text-muted hover:text-text-primary'}`}>
        ...
        <div className="mt-2 px-3 py-2 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] flex justify-between items-center">
          <p className="text-tiny text-text-muted">{payment.name} · <span className="font-mono text-text-secondary">{payment.number}</span></p>
          <p className="text-sub font-bold text-[#a78bfa]">
      ...
      {/* ── TRANSACTION ID ── */}
      <div>
        <p className="text-tiny font-black text-text-muted uppercase tracking-widest mb-2">
        ...
        <input value={transactionId} onChange={e => setTransactionId(e.target.value)}
          ...
          className={`w-full bg-[var(--bg-input)] border rounded-xl px-3 py-2.5 text-sub text-text-primary placeholder:text-text-muted outline-none transition-colors ${transactionId.trim() ? 'border-[#7c6aff]/40' : 'border-[var(--border-color)]'} focus:border-[#7c6aff]/60`} />
      </div>
        ...
        {screenshotUrl ? (
          <div className="relative rounded-xl overflow-hidden border border-[#7c6aff]/30">
            ...
            <button onClick={() => { setScreenshotUrl(null); setScreenshotFile(null); }}
              className="absolute top-2 right-2 w-6 h-6 rounded-full bg-black/70 text-white text-sub flex items-center justify-center hover:bg-black/90 transition-colors">✕</button>
          </div>
        ) : (
      ...
      {error && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-red-500/5 border border-red-500/20">
          ...
          <span className="text-caption font-bold text-red-400">{error}</span>
        </div>
      )}
      ...
      {!canSubmit && (transactionId.trim() || screenshotUrl) && !error && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-amber-500/5 border border-amber-500/20">
          <span className="text-sub">⚠️</span>
      ...
      <button onClick={handleSubmit} disabled={submitting || !canSubmit}
        className={`w-full h-11 rounded-xl font-black text-body flex items-center justify-center gap-2 transition-all ${canSubmit ? 'bg-gradient-to-r from-[#7c6aff] to-[#a78bfa] text-white shadow-lg shadow-[#7c6aff]/25 hover:shadow-[#7c6aff]/40 hover:scale-[1.01]' : 'bg-[var(--bg-input)] text-text-muted cursor-not-allowed'}`}>
        {submitting ? (
      ...
      <p className="text-center text-tiny text-text-muted">{t('premium_pkg_footer')}</p>
