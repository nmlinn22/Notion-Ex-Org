...
export const GroupSettings: React.FC<GroupSettingsProps> = ({
  ...
  const filteredAndSortedGroups = useMemo(() => {
    let result = groups.filter(g => g.toLowerCase().includes(search.toLowerCase()));
      ...
      <div className="flex-1 border border-[var(--border-color)] rounded-xl overflow-hidden bg-[var(--bg-card)] flex flex-col">
        ...
        <div className="flex items-center justify-between px-3 py-2 bg-[var(--bg-input)] border-b border-[var(--border-color)]">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold uppercase tracking-wider text-text-muted">Groups</span>
            <span className="px-1.5 py-0.5 rounded bg-[var(--bg-card)] border border-[var(--border-color)] text-[9px] text-text-muted font-mono">
              ...
              {showSortMenu && (
                <div className="absolute right-0 mt-1 w-32 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-lg shadow-xl z-50 py-1">
                  <button 
                    ...
                    className="w-full px-3 py-2 text-left text-xs hover:bg-[var(--bg-input)] flex items-center gap-2 text-text-primary"
                  >
                  ...
                  <button 
                    ...
                    className="w-full px-3 py-2 text-left text-xs hover:bg-[var(--bg-input)] flex items-center gap-2 text-text-muted"
                  >
            ...
            <tbody className="divide-y divide-[var(--border-color)]">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        <button 
                          ...
                          disabled={!newGroup}
                          className="px-2 py-1 bg-[#7c6aff] text-white rounded text-[10px] font-bold disabled:opacity-50"
                        >
                          Add
                        </button>
                        <button 
                          onClick={() => { setIsAdding(false); setNewGroup(''); }}
                          className="px-2 py-1 bg-[var(--bg-input)] text-text-muted rounded text-[10px] font-bold"
                        >
                          Cancel
                        </button>
                      </div>
              ...
              {filteredAndSortedGroups.length > 0 ? (
                filteredAndSortedGroups.map((g, idx) => (
                  <tr key={g} className="hover:bg-[var(--bg-input)]/30 transition-colors group">
                    <td className="px-3 py-2.5 text-[10px] text-text-muted border-r border-[var(--border-color)] text-center font-mono w-10">
                      {idx + 1}
                    </td>
                ...
                <tr>
                  <td className="px-4 py-10 text-center text-text-muted text-xs italic">
                    No groups found
                  </td>
                </tr>
              )}
