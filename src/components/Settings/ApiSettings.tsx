  ...
  return (
    <div className="space-y-6 pb-4">
      ...
      <div className="space-y-3">
        <p className="text-[11px] font-black text-text-muted uppercase tracking-widest">{t('api_storage_mode_label')}</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <button onClick={() => setStorageMode('app')}
            ...
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
              storageMode === 'app' ? 'bg-[#7c6aff] text-white' : 'bg-[var(--bg-card)] text-text-muted'
            }`}>
              <Zap size={18} />
            </div>
            <div>
              <p className={`text-sm font-black ${storageMode === 'app' ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'}`}>
                {t('api_app_only_title')}
              </p>
              <p className="text-[11px] text-text-muted mt-0.5 leading-relaxed">
                {t('api_app_desc')}<br />{t('api_app_no_notion')}
              </p>
            </div>
            {storageMode === 'app' && (
              <span className="text-[10px] bg-[#7c6aff] text-white font-bold px-2 py-0.5 rounded-full">{t('api_selected')}</span>
            )}
          </button>
          ...
          <button
            ...
            className={`flex flex-col items-start gap-3 p-4 rounded-2xl border-2 transition-all text-left relative overflow-hidden ${
              !canUseNotion
            ...
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
              storageMode === 'notion' && canUseNotion ? 'bg-[#7c6aff] text-white' : 'bg-[var(--bg-card)] text-text-muted'
            }`}>
              <BookOpen size={18} />
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <p className={`text-sm font-black ${storageMode === 'notion' && canUseNotion ? 'text-[var(--text-primary)]' : 'text-[var(--text-secondary)]'}`}>
                ...
                {!canUseNotion && (
                  <span className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full bg-amber-500/15 border border-amber-500/30">
                    ...
                    <span className="text-[8px] font-black text-amber-500 uppercase tracking-wide">Pro</span>
                  </span>
                )}
              </div>
              <p className="text-[11px] text-text-muted mt-0.5 leading-relaxed">
      ...
      <div className="space-y-4">
        <p className="text-[11px] font-black text-text-muted uppercase tracking-widest">{t('api_keys_label')}</p>

        {/* Gemini */}
        <div className="space-y-1.5">
          <div className="flex items-center gap-2">
            <label className="text-sm font-bold text-[var(--text-primary)]">{t('api_gemini_label')}</label>
            <span className="text-[10px] bg-[#fbbf24]/15 text-[#fbbf24] border border-[#fbbf24]/30 px-2 py-0.5 rounded-full font-bold">{t('api_required_badge')}</span>
          </div>
          <div className="relative">
            <input type={showGemini ? 'text' : 'password'} value={geminiKey}
              onChange={e => setGeminiKey(e.target.value)} placeholder="AIza..."
              className="w-full bg-[var(--bg-input)] border border-[var(--border-color)] rounded-xl p-3 pr-20 text-sm font-mono text-[var(--text-primary)] outline-none focus:border-[#7c6aff]/60 transition-all" />
        ...
        {storageMode === 'notion' && (
          <div className="space-y-4 bg-[#7c6aff]/5 border border-[#7c6aff]/20 rounded-2xl p-4">
            <p className="text-[10px] font-black text-[#a78bfa] uppercase tracking-widest">{t('api_notion_credentials')}</p>

            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <label className="text-sm font-bold text-[var(--text-primary)]">{t('api_notion_key_label')}</label>
                <span className="text-[10px] bg-[#fbbf24]/15 text-[#fbbf24] border border-[#fbbf24]/30 px-2 py-0.5 rounded-full font-bold">{t('api_required_badge')}</span>
              </div>
              <div className="relative">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center gap-2">
                <label className="text-sm font-bold text-[var(--text-primary)]">{t('api_database_id_label')}</label>
                <span className="text-[10px] bg-[#fbbf24]/15 text-[#fbbf24] border border-[#fbbf24]/30 px-2 py-0.5 rounded-full font-bold">{t('api_required_badge')}</span>
              </div>
              <div className="relative">
      ...
      <button onClick={handleSave}
        className={`w-full h-12 rounded-xl text-white text-sm font-bold transition-all shadow-lg ${
          saved
