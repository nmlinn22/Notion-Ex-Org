  ...
  return (
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-xs font-black text-[var(--text-primary)]">{t('settings_backup')}</p>
          <p className="text-[10px] text-text-muted">{t('backup_count', { count: backups.length })}</p>
        </div>
        <button
          onClick={isPremium ? handleCreateBackup : () => onOpenPremium?.()}
          disabled={isPremium && isSaving}
          className="flex items-center gap-1.5 px-3 h-9 rounded-xl bg-[#7c6aff] text-white text-[11px] font-bold hover:bg-[#a78bfa] transition-all disabled:opacity-50"
        >
          {isPremium && isSaving
      ...
      <div className="bg-[var(--bg-input)] border border-[var(--border-color)] rounded-xl p-3 space-y-2">
        <p className="text-[10px] font-bold text-text-muted uppercase tracking-wider">{t('backup_restore_mode_label')}</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-10 text-text-muted text-xs">
          ...
          {t('backup_empty')}<br />
          <span className="text-[10px]">{t('backup_empty_hint')}</span>
        </div>
      ) : (
          ...
          {backups.map(backup => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[11px] font-bold truncate">{backup.label}</p>
                <p className="text-[10px] text-text-muted">
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              <div className={`absolute top-0 left-0 w-full h-1 ${
                confirmAction.type === 'delete' || (confirmAction.type === 'restore' && confirmAction.mode === 'replace')
              ...
              <div className="flex flex-col items-center text-center space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <p className="text-[13px] font-semibold text-[#a78bfa]">{confirmAction.backup.label}</p>
                ...
                <div className="w-full flex gap-2 pt-1">
                  <button onClick={() => setConfirmAction(null)}
                    className="flex-1 h-11 rounded-xl bg-[var(--bg-input)] border border-[var(--border-color)] text-text-muted font-bold text-sm hover:text-text-primary transition-all">
                    {t('btn_cancel')}
                  </button>
                  <button
                    onClick={confirmAction.type === 'delete' ? handleDelete : handleRestore}
                    className={`flex-1 h-11 rounded-xl text-white font-bold text-sm transition-all shadow-lg ${
                      confirmAction.type === 'delete' || confirmAction.mode === 'replace'
