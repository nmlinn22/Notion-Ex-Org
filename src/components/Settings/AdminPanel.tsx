    ...
    <>
      ...
      <AnimatePresence>
        {dashboardToast && (
          ...
          >
            <CheckCircle size={15} />
            <span className="text-sub font-semibold">{dashboardToast}</span>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex gap-1 mb-4 p-1 bg-[var(--bg-input)] rounded-xl border border-[var(--border-color)]">
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-sub font-semibold transition-all ${activeTab === 'users'
        ...
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-sub font-semibold transition-all relative ${activeTab === 'payments'
              ? 'bg-[var(--bg-card)] text-[#a78bfa] shadow'
              : 'text-text-muted hover:text-text-primary'
            }`}
        >
          ...
          {pendingPaymentCount > 0 && (
            <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-1 bg-red-500 text-white text-tiny font-semibold rounded-full flex items-center justify-center">
              {pendingPaymentCount}
            </span>
          )}
        </button>
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-sub font-semibold transition-all ${activeTab === 'broadcast'
              ? 'bg-[var(--bg-card)] text-[#a78bfa] shadow'
              : 'text-text-muted hover:text-text-primary'
            }`}
        >
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-3 gap-2">
            {[
              ...
              <div key={s.label} className="bg-[var(--bg-input)] border border-[var(--border-color)] rounded-xl p-3 text-center">
                ...
                <div className="text-tiny text-text-muted font-semibold mt-0.5">{s.label}</div>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-xl border border-[var(--border-color)] overflow-visible">
            {filteredUsers.length === 0 ? (
              <div className="text-center py-10 text-text-muted text-sub italic">{t('admin_no_users')}</div>
              ...
              return (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-caption font-semibold text-text-primary truncate">{u.email}</p>
                    <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                      {isSelf && (
                        <span className="text-tiny bg-[#7c6aff]/15 text-[#a78bfa] font-bold px-1.5 py-0.5 rounded-full">
                          {t('admin_badge_you')}
                        </span>
                      )}
                      {/* Role badge */}
                      <span className="text-tiny font-bold px-1.5 py-0.5 rounded-full flex items-center gap-0.5"
                      ...
                      {/* Entry count badge */}
                      {userStats[u.id] && (
                        <span className="text-tiny font-bold px-1.5 py-0.5 rounded-full"
                  ...
                  {!isSelf && (
                    <button
                      ...
                      className="ml-3 px-3 py-1.5 bg-[var(--bg-input)] border border-[var(--border-color)] rounded-lg text-caption font-semibold text-text-muted hover:text-text-primary hover:border-[#7c6aff]/40 transition-all shrink-0"
                    >
                      {t('admin_manage_btn')}
                    </button>
                  )}
      ...
      {activeTab === 'broadcast' && (
        <div className="space-y-3 bg-[var(--bg-input)] p-4 rounded-xl border border-[var(--border-color)]">
          <div>
            <h3 className="text-body font-bold text-text-primary">System Broadcast</h3>
            <p className="text-tiny text-text-muted mt-1">Send a notification to ALL users instantly.</p>
          </div>
          <textarea
            value={broadcastMessage}
            onChange={(e) => setBroadcastMessage(e.target.value)}
            placeholder="Type your message here..."
            className="w-full bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-3 text-body min-h-[100px] outline-none focus:border-[#7c6aff]/50 transition-all resize-none"
          />
          <button
            onClick={handleBroadcast}
            disabled={isBroadcasting || !broadcastMessage.trim()}
            className="w-full h-10 bg-[#7c6aff] text-white rounded-xl font-bold text-sub hover:bg-[#8b7aff] transition-all disabled:opacity-50"
          >
