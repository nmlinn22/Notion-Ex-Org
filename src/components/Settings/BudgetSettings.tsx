...
export const BudgetSettings: React.FC<BudgetSettingsProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── Section 1: {t('budget_group_limits')} — compact ── */}
      {budgetTab === 'limits' && (
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-2xl overflow-hidden">
        <div className="px-4 py-2.5 border-b border-[var(--border-color)] bg-[var(--bg-input)]">
          <p className="text-[10px] font-bold text-text-muted uppercase tracking-wider">{t('budget_group_limits')}</p>
        </div>
        <div className="divide-y divide-[var(--border-color)]">
          {groups.map((g) => {
            ...
            const barColor = over ? '#f87171' : pct > 75 ? '#fbbf24' : '#34d399';

            return (
              <div key={g} className="px-4 py-3">
                {isEditing ? (
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-[var(--text-primary)] shrink-0 min-w-[80px]">{g}</span>
                    <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
                      placeholder={t('budget_limit_placeholder')} autoFocus onKeyDown={e => e.key === 'Enter' && handleSave()}
                      className="flex-1 bg-[var(--bg-input)] border border-[#7c6aff]/40 rounded-xl px-3 py-1.5 text-sm font-mono outline-none focus:border-[#7c6aff] text-[var(--text-primary)]" />
                    <button onClick={handleSave} className="p-1.5 rounded-lg bg-[#7c6aff] text-white"><Check size={13} /></button>
                    <button onClick={() => { setEditing(null); setAmount(''); }}
                      className="p-1.5 rounded-lg bg-[var(--bg-input)] border border-[var(--border-color)] text-text-muted"><X size={13} /></button>
                  </div>
                ) : (
                  <>
                    <div className="flex items-center justify-between gap-2">
                      <span className="text-sm font-bold text-[var(--text-primary)] truncate">{g}</span>
                      <div className="flex items-center gap-2 shrink-0">
                        {budget ? (
                          <span className={`text-xs font-mono font-bold ${over ? 'text-[#f87171]' : 'text-[#a78bfa]'}`}>
                            {fmt(spent)}<span className="text-text-muted font-normal"> / {fmt(budget.amount)}</span>
                          </span>
                        ) : (
                          <span className="text-[11px] text-text-muted italic">{t('budget_no_limit')}</span>
                        )}
                        <button onClick={() => startEdit(g)}
                          className="p-1 rounded-lg bg-[#7c6aff]/10 text-[#a78bfa] hover:bg-[#7c6aff]/20 transition-all">
                          <Edit2 size={11} />
                        </button>
                    ...
                    {budget && (
                      <div className="mt-1.5">
                        ...
                        <div className="flex items-center justify-between mt-0.5">
                          <span className={`text-[9px] ${over ? 'text-[#f87171] font-bold' : 'text-text-muted'}`}>
                            {over ? `⚠ Over by ${fmt(spent - budget.amount)}` : `${pct}% ` + t('budget_used')}
                          </span>
                          <span className="text-[9px] text-text-muted">{fmt(Math.max(0, budget.amount - spent))} left</span>
                        </div>
                      </div>
                    )}
                  </>
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="relative">
            <button onClick={() => setYearOpen(o => !o)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[var(--bg-input)] border border-[var(--border-color)] text-sm font-bold hover:border-[#7c6aff]/50 transition-all">
              {tableYear}
                ...
                <div className="absolute right-0 top-full mt-1 z-50 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl shadow-xl overflow-hidden min-w-[80px]">
                  {YEARS.map(y => (
                    <button key={y} onClick={() => { setTableYear(y); setYearOpen(false); }}
                      className={`w-full px-4 py-2 text-left text-sm font-semibold transition-colors ${y === tableYear ? 'bg-[#7c6aff]/15 text-[#a78bfa]' : 'text-[var(--text-primary)] hover:bg-[var(--bg-hover)]'}`}>
                      {y}
                    </button>
                  ))}
                </div>
